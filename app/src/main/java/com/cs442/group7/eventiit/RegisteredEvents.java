...
public class RegisteredEvents extends BaseActivity implements View.OnClickListener,UploadDownloadUtil.GenericListener {
    ...
    private static String url = "http://ec2-54-147-231-178.compute-1.amazonaws.com/PhpProject1/get_registered_events.php";
    ...
    private boolean intentForViewEvent;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        intentForViewEvent = false;
        if(getIntent().getStringExtra("eventID") != null){
            intentEventID = (getIntent().getStringExtra("eventID"));
            intentForViewEvent = true;
        }
...
//        top_registered_events.setOnClickListener(this);
        ...
        top_registered_events.setTextColor(Color.parseColor("#FFFFFF"));

        listView = (ListView) findViewById(R.id.listview_this_week);
    ...
    public void onTaskComplete(boolean success, List<Map<String, String>> response, boolean error, String passThroughParam) {
        ...
        if(success) {
            ...
            // if clicked from notification...
            if(intentForViewEvent==true){


//                Log.e(TAG,"eventID "+intentEventID);
//                Log.e(TAG, "event Item Array Size "+eventItemModelArrayList.size());

                int position=-1;

                for(int i = 0; i<eventItemModelArrayList.size(); i++)
                {
                    if(intentEventID.equals(eventItemModelArrayList.get(i).getEventID())){
                        position = i;
//                        Log.e(TAG, "Position = i = "+i );
                        intentForViewEvent=false;
                ...
                EventItemModel eim = eventItemModelArrayList.get(position);
